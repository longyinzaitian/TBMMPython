...
#                              '<div.*?class="d_post_content j_d_post_content.*?>(.*?)</div>.*?</cc>.*?'+
...
#         pattern = re.compile('<div class="d_author">(.*?)</div>',re.S)
        ...
        list = []
        tail2 = re.compile('<span class="tail-info">(.*?)</span>.*?<span class="tail-info">(.*?)</span>', re.S)
        tail3 = re.compile('<span class="tail-info">.*?</span>.*?<span class="tail-info">(.*?)</span>.*?<span class="tail-info">(.*?)</span>',re.S)
        import sys;reload(sys);sys.setdefaultencoding('utf8');
        for item in content:
            n = re.subn('tail-info', 'tail', item[2])
            if n[1]==4:
                group3 = re.search(tail3, item[2])
                list.append((item[0],item[1],group3.group(1),group3.group(2)))
            elif n[1]==3:
                group2 = re.search(tail2, item[2])
                list.append((item[0],item[1],group2.group(1),group2.group(2)))
            else:
    ...
    def start(self):
        ...
        self.writeTxt(num.group(1)+' post number,'+num.group(2)+'page number'+os.linesep)
        print u'帖子标题:%s' %self.title
        
        for i in range(int(self.pagenum)):
           pagecontent = self.getPageContent(\
           ...
           for con in content:
               ...
               self.writeTxt(strcon+os.linesep)
